...
public class Account {
    ...
    long withdraw(long amount) throws BalanceInsufficientException {// 잔액 안에서 출금허용
        if (amount > balance) {
            // return; -> void가 아니여서 사용불가
            throw new BalanceInsufficientException("잔액부족");// Exception 즉, 오류를 발생시켜서 아래 실행하지 않고 멈춤
        }
        balance -= amount;
        return balance;
    }
